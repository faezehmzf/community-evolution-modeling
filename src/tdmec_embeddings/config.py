...
class EmbeddingRunConfig:
    ...
    def validate(self) -> None:
        ...
        if self.force:
            raise EmbeddingConfigError(
                "force overwrite of completed outputs is disabled by policy; "
                "use replace_incomplete only for interrupted/failed runs"
            )
        if self.replace_incomplete and self.resume:
            raise EmbeddingConfigError(
                "replace_incomplete and resume are mutually exclusive"
            )
...
def load_embedding_config(path: str | Path) -> EmbeddingRunConfig:
    ...
    config = EmbeddingRunConfig(
        ...
        replace_incomplete=bool(root.get("replace_incomplete", False)),
